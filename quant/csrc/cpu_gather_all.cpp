...
inline void gather_cpu(
    ...
    uint8_t* __restrict__ output_signs,
        ...
        for(int j = 0; j < index_head; ++j) {
            ...
            for(int k=0; k<index_len; k+=4) {
                ...
                for(int uk=0; uk<4; ++uk) {
                    ...
                    __m128i data = _mm_loadu_si128((const __m128i*)src_sign);
                    _mm_stream_si128((__m128i*)dst_sign, data);
                }
            }
        }
    }
...
void my_cpu_gather(const torch::Tensor quant_kv,const torch::Tensor quant_param,const torch::Tensor signs, const torch::Tensor& index,torch::Tensor output_kv,torch::Tensor output_param,torch::Tensor output_signs) {
    ...
    gather_cpu(quant_kv.data_ptr<int32_t>(),quant_param.data_ptr<c10::Half>(),signs.data_ptr<uint8_t>(),
        index.data_ptr<int64_t>(),output_kv.data_ptr<int32_t>(),output_param.data_ptr<c10::Half>(),output_signs.data_ptr<uint8_t>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    py::arg("quant_kv"),py::arg("quant_param"),py::arg("signs"), py::arg("index"),py::arg("output_kv"),py::arg("output_param"),py::arg("output_sign"));
}