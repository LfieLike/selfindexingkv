...
__global__ void quantize_with_outliers_kernel(
    ...
    if(th_id<128){
        share_key_channle[th_id] = __half2float(key_channle[batch_id * head_size*128+head_size*128+th_id]);
        share_value_channle[th_id] = __half2float(value_channle[batch_id * head_size*128+head_size*128+th_id]);
    }
    __syncthreads();
    // 判断边界
