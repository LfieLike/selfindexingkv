...
__global__ void quantize_with_outliers_kernel(
    ...
    c10::Half* res,
    // outlier channel的数目
    int batch_stride,int head_stride,int token_stride,int lut_size,int batch_size,int head_size,int len
    ) {
    ...
    // 获取当前线程处理的vec的起始位置
    int base_index = (batch_id * batch_stride) 
                   + (head_id * head_stride) 
                   + ((pro_id * NUM_PER_THREAD+vec_id)*token_stride);
    ...
    reinterpret_cast<int4 *>(&onebit_key_set[0])[0] = reinterpret_cast<int4 *>(&key_states[base_index])[0];
    #pragma unroll
    for(int i=0;i<16;i++){
        // int pos = (th_id+i)%16;
...
torch::Tensor MyScoreCudaTemplate(
    ...
    int batch_stride = key_states.stride(0);
    int head_stride = key_states.stride(1);
    int token_stride = key_states.stride(2);
...
//     Compiler hints for using L2 Persistent Cache
    ...
    batch_stride,head_stride,token_stride,lut_size,batch,head,len);
