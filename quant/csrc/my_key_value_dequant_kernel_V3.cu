...
// batch_id和head_id不能*NUM_PER_THREAD*WARPS_PER_BLOCK 来表示位移，因为可能会有越界的线程。
    ...
    // if(th_id<32){
    //     printf("quant_param_offset:%d,local_scale_key:%f,local_zp_key:%f\n",quant_param_offset,local_scale_key,local_zp_key);
    // }
    #pragma unroll
    for(int i=0;i<8;i++){
    ...
    reinterpret_cast<int4*>(value_states+base_index_key)[0] = reinterpret_cast<int4*>(value_half_data)[0];
    reinterpret_cast<int4*>(key_states+base_index_key)[0] = reinterpret_cast<int4*>(key_half_data)[0];
    return;
}
