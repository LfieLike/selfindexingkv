...
__global__ void quantize_with_outliers_kernel(
    ...
    if(pro_id * NUM_PER_THREAD*EMB_DIM + vec_id*128>=len*EMB_DIM){
        return;
    }
    int lut_offset = batch_id*head_size*lut_size + head_id*lut_size +sub_th_id*32;
    ...
    float sub_res = __ldg(&query_lut[lut_offset+(onebit_key&15)])+__ldg(&query_lut[lut_offset+16+(onebit_key>>4)]);
    // printf("lut_offset:%d,sub_res:%f= %f+%f\n",lut_offset,sub_res,query_lut[lut_offset+(onebit_key&15)],query_lut[lut_offset+16+(onebit_key>>4)]);
