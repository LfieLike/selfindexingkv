...
__global__ void quantize_with_outliers_kernel(
    uint8_t*  key_quantized_data,
    uint8_t*  value_quantized_data,
    // 压缩好的数据
    T*  key_dequant_dst,
    T*  value_dequant_dst,
    T* key_quant_zp,
    ...
    int quant_shift = (quant_base_index+vec_id*128+sub_th_id*8)/4;
    ...
    uint8_t key_quant_cache[4];
    uint8_t value_quant_cache[4];
    int idx;
    for(int i=0 ;i<4;i++){
        key_quant_cache[i] = key_quantized_data[quant_shift+i];
        value_quant_cache[i] = value_quantized_data[quant_shift+i];
    }
    idx = quant_idx[idx_base_index+vec_id];
    // idx 记录了当前是原始cache中的第几行, 还要加上当前子线程的shift
    int true_shift_dequant = dequant_base_index+idx*EMB_DIM+sub_th_id*8;
    for(int i=0;i<4;i++){
        key_dequant_dst[true_shift_dequant+i] = static_cast<float>((key_quant_cache[0]>>i)&0x03);
        value_dequant_dst[true_shift_dequant+i] = static_cast<float>((value_quant_cache[0]>>i)&0x03);
    }
    for(int i=0;i<4;i++){
        key_dequant_dst[true_shift_dequant+i+4] = static_cast<float>((key_quant_cache[1]>>i)&0x03);
        value_dequant_dst[true_shift_dequant+i+4] = static_cast<float>((value_quant_cache[1]>>i)&0x03);
    }
    return;
}
...
void MyScatterAddCudaTemplate(
    torch::Tensor  key_quantized_data,
    torch::Tensor  value_quantized_data,
...
//     Compiler hints for using L2 Persistent Cache
    ...
    key_quantized_data.data_ptr<uint8_t>(),
    value_quantized_data.data_ptr<uint8_t>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    py::arg("key_quantized_data"),
    py::arg("value_quantized_data"),
