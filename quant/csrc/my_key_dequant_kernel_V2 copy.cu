...
// batch_id和head_id不能*NUM_PER_THREAD*WARPS_PER_BLOCK 来表示位移，因为可能会有越界的线程。
    ...
    // 获取量化参数，首先需要读取index
    int64_t now_index = index[batch_id* head_size * len+head_id * len+vec_id];
    // printf("offset:%lld\n",now_index);
    // 再根据index读取 1bitmask，还有zp scale
    uint8_t* key_states_1bit_ptr = key_states_1bit + (batch_id * head_size * len_1bit * EMB_DIM+head_id * len_1bit * EMB_DIM + now_index*EMB_DIM)/8;
    uint8_t now_key_states_1bit = key_states_1bit_ptr[0];
    // zp 和 scale 是 每32个向量共享的
    int len_zp = len_1bit/32;
    int index_zp = now_index/32;
    T* zp_ptr = zp + (batch_id * head_size * len_zp * EMB_DIM+ head_id * len_zp * EMB_DIM) + index_zp*EMB_DIM+(th_id*NUMBER_PER_THREAD)%EMB_DIM;
    T* scale_ptr = scale + (batch_id * head_size * len_zp * EMB_DIM+ head_id * len_zp * EMB_DIM) + index_zp*EMB_DIM+(th_id*NUMBER_PER_THREAD)%EMB_DIM;
    // 最后是2bit的偏移量 
    ...
    for(int i=0;i<4;++i){
        ...
        dequant_result[i] = ((now_key_states_1bit>>i)&1) ? (quant_res-now_zp)*now_scale : -(quant_res-now_zp)*now_scale;
        if(th_id==0){
            printf("dequant:%f,quant_res:%f,zp:%f,scale:%f\n",dequant_result[i],quant_res,now_zp,now_scale);
        }
    }
    ...
    for(int i=0;i<4;++i){
        ...
        dequant_result[i+4] = ((now_key_states_1bit>>(i+4))&1) ? (quant_res-now_zp)*now_scale : -(quant_res-now_zp)*now_scale;
        if(th_id==0){
            printf("dequant:%f,quant_res:%f,zp:%f,scale:%f\n",dequant_result[i+4],quant_res,now_zp,now_scale);
        }
    }
    #pragma unroll
    for(int i=0;i<8;++i){
...
void MyKeyDeQuantCudaTemplate(
    ...
    int len_1bit = key_states_2bit.size(2);
