...
__global__ void quantize_with_outliers_kernel(
    ...
    T*  key_dequant_dst,
    T*  value_dequant_dst,
    //     quant_param 的存储顺序为：T* key_quant_zp,
    //      T* key_quant_scale, 
    //      T* value_quant_zp,
    //      T* value_quant_scale, 
    float* quant_param,
    ...
    int num,
    // num 是要解压多少个
    int batch_size, int head_size, int len ,int key_buffer_len,int value_buffer_len
    // len是总长度
    ) {
    ...
    // 表示dequant_dst的基础位置
    int key_dequant_base_index = (batch_id * head_size * key_buffer_len * EMB_DIM) 
                   + (head_id * key_buffer_len * EMB_DIM);
    int value_dequant_base_index = (batch_id * head_size * value_buffer_len * EMB_DIM) 
                   + (head_id * value_buffer_len * EMB_DIM);
    // 获取当前是解压哪一个
    ...
    int key_true_shift_dequant = key_dequant_base_index+idx*EMB_DIM+sub_th_id*8;
    int value_true_shift_dequant = value_dequant_base_index+idx*EMB_DIM+sub_th_id*8;
    ...
    reinterpret_cast<int4*>(key_dequant_cache)[0] = reinterpret_cast<int4*>(key_dequant_dst+key_true_shift_dequant)[0];
    reinterpret_cast<int4*>(value_dequant_cache)[0] = reinterpret_cast<int4*>(value_dequant_dst+value_true_shift_dequant)[0];
    float quant_param_cache[4];
    reinterpret_cast<float4*>(quant_param_cache)[0] = reinterpret_cast<float4*>(quant_param+(idx_base_index+pro_id*8+vec_id)*4)[0];
    float key_zp =  quant_param_cache[0];
    float key_scale =  quant_param_cache[1];
    float value_zp =  quant_param_cache[2];
    float value_scale =  quant_param_cache[3];
    // float key_zp =  0;
    // float key_scale =  1;
    // float value_zp =  0;
    // float value_scale =  1;
    ...
    for(int i=0;i<8;i++){
        ...
        temp += ((static_cast<float>((key_value_quant_cache[i]>>4) & 0x0F)-key_zp)*key_scale);  // 执行加法
        key_dequant_cache[i] = 	__float2half(temp);  // 将结果转换回 c10::Half 并赋值回去
        temp = __half2float(value_dequant_cache[i]);  // 将 c10::Half 转换为 float
        temp += ((static_cast<float>((key_value_quant_cache[i])&0x0F)-value_zp)*value_scale);  // 执行加法
        value_dequant_cache[i] = 	__float2half(temp);
    }
    reinterpret_cast<int4*>(key_dequant_dst+key_true_shift_dequant)[0] = reinterpret_cast<int4*>(key_dequant_cache)[0];
    reinterpret_cast<int4*>(value_dequant_dst+value_true_shift_dequant)[0] = reinterpret_cast<int4*>(value_dequant_cache)[0];
    return;
}
...
void MyScatterAddCudaTemplate(
    ...
    int batch = key_value_quantized_data.size(0);
    int head = key_value_quantized_data.size(1);
    int key_buffer_len = key_dequant_dst.size(2);
    int value_buffer_len = value_dequant_dst.size(2);
...
//     Compiler hints for using L2 Persistent Cache
    ...
    num,batch,head,len,key_buffer_len,value_buffer_len);
