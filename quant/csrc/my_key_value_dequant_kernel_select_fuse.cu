...
__global__ void quantize_with_outliers_kernel(
    ...
    T* value_channle,
    int64_t* index,
    int batch_size, int head_size, int len, int index_len,int full_len
    ) {
    ...
    int base_index_key = (batch_id * head_size * len * EMB_DIM) //batch 维度
                   ...
                   + (vec_id * EMB_DIM); 
    __shared__ float share_key_channle[128];
    __shared__ float share_value_channle[128];

    share_key_channle[th_id%128] = __half2float(key_channle[batch_id * head_size*128+head_id*128+th_id%128]);
    share_value_channle[th_id%128] = __half2float(value_channle[batch_id * head_size*128+head_id*128+th_id%128]);
    ...
    // }
    int write_offset = ((batch_id * head_size * (index_len+ full_len)) //batch 维度
                   + (head_id * (index_len + full_len)) 
                   + pro_id * THREAD_NUM 
                   + th_id + full_len) * EMB_DIM ;
    // if(pro_id == 0 && th_id == 0){
    ...
    for(int offset = 0;offset <128;offset+=8){
        ...
        for(int i=0;i<8;i++){
            ...
            // printf("shift:%d\n",offset+i)
            key_half_data[i] = __float2half(quant_res*(share_key_channle[offset+i]));
            ...
            value_half_data[i] = __float2half(value_quant_res*(share_value_channle[offset+i]));
        }
...
std::tuple<torch::Tensor, torch::Tensor> MyQuantCudaTemplate(    
    ...
    torch::Tensor channel_key,
    torch::Tensor channel_value,
    torch::Tensor index,
    int full_num
    ) {
    ...
    channel_key.data_ptr<T>(),
    channel_value.data_ptr<T>(),
    index.data_ptr<int64_t>(),
    batch,head,len,index_len,full_num);
