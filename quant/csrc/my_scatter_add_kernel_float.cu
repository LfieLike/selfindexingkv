...
// dst half 存储最终结果。 输入数据需要包括，压缩后的数值，zp scale，以及他们对应的位置（每个位置需要搬运128个数）
    ...
    // key_dequant_dst.data_ptr<float>(),
    // value_dequant_dst.data_ptr<float>(),
    // quant_param.data_ptr<float>(),
    // quant_idx.data_ptr<int>(),
    // num,batch,head,len
__global__ void quantize_with_outliers_kernel_float(
    uint8_t*  key_value_quantized_data,
    // 压缩好的数据
    float*  key_dequant_dst,
    float*  value_dequant_dst,
    float* quant_param,
    int* quant_idx,
    // outlier的索引
    int num,
    ...
    float key_dequant_cache[8];
    float value_dequant_cache[8];
    int idx;
    ...
    reinterpret_cast<int4*>(key_dequant_cache)[1] = reinterpret_cast<int4*>(key_dequant_dst+true_shift_dequant)[1];
    reinterpret_cast<int4*>(value_dequant_cache)[1] = reinterpret_cast<int4*>(value_dequant_dst+true_shift_dequant)[1];
    ...
    for(int i=0;i<8;i++){
        float temp = key_dequant_cache[i];  // 将 c10::Half 转换为 float
        temp += ((static_cast<float>((key_value_quant_cache[i]>>4) & 0x0F)-key_zp)*key_scale);  // 执行加法
        key_dequant_cache[i] = 	temp;  // 将结果转换回 c10::Half 并赋值回去
        temp = value_dequant_cache[i];  // 将 c10::Half 转换为 float
        temp += ((static_cast<float>((key_value_quant_cache[i])&0x0F)-value_zp)*value_scale);  // 执行加法
        value_dequant_cache[i] = 	temp;
    }
    ...
    reinterpret_cast<int4*>(key_dequant_dst+true_shift_dequant)[1] = reinterpret_cast<int4*>(key_dequant_cache)[1];
    reinterpret_cast<int4*>(value_dequant_dst+true_shift_dequant)[1] = reinterpret_cast<int4*>(value_dequant_cache)[1];
    return;
}
...
//     Compiler hints for using L2 Persistent Cache
    ...
    quantize_with_outliers_kernel_float<<<numBlocks, threadsPerBlockDim, 0>>>(
    key_value_quantized_data.data_ptr<uint8_t>(),
    key_dequant_dst.data_ptr<float>(),
    value_dequant_dst.data_ptr<float>(),
    quant_param.data_ptr<float>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("My_scatter_add_float", &MyScatterAddCudaTemplate, "Quantize using float precision",
