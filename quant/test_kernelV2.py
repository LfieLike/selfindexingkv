...
@gpu_timer
def myquant(key_states,zp,scale,group_size):
    return cuda_my_key_quant_V2.My_key_quant_V2(
        key_states,zp,scale,group_size
    )
@gpu_timer
def mydequant(key_states_1bit,key_states_2bit,dequant_dst,zp,scale,index,group_size):
...
def test_quantize_with_outliers():
    ...
    key_states = -torch.rand((batch_size, head_size, seq_len, emb_dim), device='cuda', dtype=torch.float16)
    _,indexcnt = torch.topk(key_states.mean(dim=-1,keepdim=True),k=20,dim=-2)
    expand_index = indexcnt.expand(-1,-1,-1,128)
    # print(index.shape)
    # 创建 outlier 相关的张量
    # outlier_idx = torch.randint(0, emb_dim, (batch_size, head_size,1,outlier_num), device='cuda', dtype=torch.uint8)
    shape = key_states.shape
    key_states_needquant = key_states.view(shape[0],shape[1],shape[2]//32,32,shape[3])
    ...
    quantized = torch.round((trnas_key_states.float()-zp.float())/scale.float())
    ...
    dequantizedV2 = transposequant(key_states)
    print("error:",(dequantized-dequantizedV2).abs().mean())
    # index = torch.range(0,shape[-2]-1).expand(shape[0],shape[1],-1).to(torch.int64).cuda()
    # print(index.shape)
    for i in range(5):
        key_1bit,key_2bit = myquant(
            key_states,
            zp,scale,group_size=32
        )
        compressed_signs = extract_sign_and_compress(key_states)
        res = torch.zeros_like(key_states).view(-1)
        select_key_2bit = torch.gather(key_2bit,-2,indexcnt.expand(-1,-1,-1,32))
        print(select_key_2bit.shape,key_1bit.shape,res.shape,zp.shape,scale.shape,indexcnt.shape)
        mydequant(key_1bit,select_key_2bit,res,zp,scale,indexcnt,32)
    print("----")
    # quant_key=(key_states-outlier_zp.unsqueeze(-2))/(outlier_scale.unsqueeze(-2))
    print("1bit量化误差:", (compressed_signs-key_1bit.view(-1)).sum())
    select_true_quant = torch.gather(dequantized,2,indexcnt.expand(-1,-1,-1,128))
    print((res[:select_true_quant.numel()]-select_true_quant.view(-1)).abs().mean())
