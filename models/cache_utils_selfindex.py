...
def draw(matrix):
    ...
    n, bins, patches = plt.hist(values, bins=200, density=True, edgecolor='black', alpha=0.7)

    # 绘制高斯分布拟合曲线
    xmin, xmax = plt.xlim()
    x = np.linspace(xmin, xmax, 100)
    p = norm.pdf(x, mu, sigma)
    plt.plot(x, p, 'k', linewidth=2, label=f'拟合高斯曲线: μ={mu:.2f}, σ={sigma:.2f}')

    # 添加标题和标签
    plt.title('矩阵值分布直方图与高斯分布拟合')
    plt.xlabel('值')
    plt.ylabel('概率密度')
    ...
    # 保存图片
    plt.savefig('matrix_distribution.png', dpi=300, bbox_inches='tight')
...
class selfindexCache(Cache):
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            self.select_len = int(0.075*need_compress_valuecache.shape[-2])
            ...
            _, now_indices = torch.topk(score_now,k=min(self.select_len,score_now.shape[-1]),dim = -1,sorted=False)
...
class selfindex_16bit_Cache(Cache):
    """
    A cache that grows dynamically as more tokens are generated. This is the default for generative models.
...
class questCache(Cache):
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            if layer_idx == 0:
                prompt_len = key_states.shape[-2]
                self.neaset_len = prompt_len - ((prompt_len-self.neaset_len)//16)*16
                # print(prompt_len,self.neaset_len)
            need_compress_keycache = key_states[:,:,:-self.neaset_len,:].clone()
            need_compress_valuecache = value_states[:,:,:-self.neaset_len,:].clone()
            shape = need_compress_keycache.shape
            self.max_key.append(need_compress_keycache.view(shape[0],shape[1],-1,self.chunk_size,shape[-1]).max(dim=-2)[0])
            self.min_key.append(need_compress_keycache.view(shape[0],shape[1],-1,self.chunk_size,shape[-1]).min(dim=-2)[0])
            self.compress_key_cache.append(need_compress_keycache)
            self.compress_value_cache.append(need_compress_valuecache)
            self.key_cache.append(key_states[:,:,-self.neaset_len:,:])
            self.value_cache.append(value_states[:,:,-self.neaset_len:,:])
            self.select_len = int(0.075*need_compress_valuecache.shape[-2])
            return key_states,value_states
        else:
            self.key_cache[layer_idx] = torch.cat([self.key_cache[layer_idx], key_states], dim=-2)
            ...
            q_1 = query_states[:,:,-1:,:]
            shape = q_1.shape
            query_states = q_1.view(shape[0],key_states.shape[1],-1,shape[2],shape[3]).mean(dim=2)

            A = self.max_key[layer_idx]* query_states
            B = self.min_key[layer_idx]* query_states
            result = torch.max(A, B)
            score_now = result.sum(dim=-1)
            # print(score_now.shape)
            _, now_indices = torch.topk(score_now,k=min(self.select_len//self.chunk_size,score_now.shape[-1]),dim = -1,sorted=False)
            
            offset = torch.arange(self.chunk_size,device=now_indices.device,dtype=now_indices.dtype)
            expanded_now_indices = now_indices.view(shape[0],key_states.shape[1],-1,1).expand(-1,-1,-1,self.chunk_size)*self.chunk_size
            # 将 offset 与 expanded_now_indices 相加
            # 由于广播机制，offset 会被扩展到与 expanded_now_indices 相同的形状进行相加
            now_indices = expanded_now_indices + offset
            # print(now_indices)
            now_indices = now_indices.view(shape[0],key_states.shape[1],-1,1).expand(-1,-1,-1,128)
            cnt_k = torch.gather(self.compress_key_cache[layer_idx],dim=-2,index=now_indices)
            cnt_v = torch.gather(self.compress_value_cache[layer_idx],dim=-2,index=now_indices)
            # print(cnt_k.shape)
        return torch.cat((self.key_cache[layer_idx],cnt_k),dim=-2), torch.cat((self.value_cache[layer_idx],cnt_v),dim=-2)
    def return_given_alpha(alpha, sort_res, W_metric, tmp_metric, sum_before):
        thres_cumsum = sum_before * alpha 
        sort_mask = tmp_metric <= thres_cumsum.reshape((-1,1))
        thres = torch.gather(sort_res[0], dim=1, index=sort_mask.sum(dim=1, keepdims=True)-1)
...
class dsCache(Cache):
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            if layer_idx == 0:
                prompt_len = key_states.shape[-2]
                # self.neaset_len = prompt_len - ((prompt_len-self.sink_len-self.neaset_len)//64)*64 -self.sink_len
                # print(prompt_len,self.neaset_len)
            q_1 = query_states
            shape = q_1.shape
            q_1 = q_1.view(shape[0],key_states.shape[1],-1,shape[2],shape[3]).mean(dim=2)
            k_1 = key_states
            _,index = torch.topk(q_1.mean(dim=-2,keepdim=True).abs()*key_states.abs().max(dim=-2,keepdim=True)[0],k=16,dim=-1)
            a = torch.zeros_like(q_1.mean(dim=-2,keepdim=True))
            a.scatter_(-1,index,1)
            self.mask.append(a)
            need_compress_keycache = key_states[:,:,self.sink_len:-self.neaset_len,:].clone()
            need_compress_valuecache = value_states[:,:,self.sink_len:-self.neaset_len,:].clone()
            self.key_cache.append(torch.cat((key_states[:,:,-self.neaset_len:,:],key_states[:,:,:self.sink_len,:]),dim=-2))
            self.value_cache.append(torch.cat((value_states[:,:,-self.neaset_len:,:],value_states[:,:,:self.sink_len,:]),dim=-2))
            self.compress_key_cache.append(need_compress_keycache)
            self.compress_value_cache.append(need_compress_valuecache)
            self.select_len = int(0.075*need_compress_valuecache.shape[-2])
            return key_states,value_states
            ...
            shape = q_1.shape
            query_states = q_1.view(shape[0],key_states.shape[1],-1,shape[2],shape[3]).mean(dim=2)
            score_now = torch.matmul(self.compress_key_cache[layer_idx],(self.mask[layer_idx]*query_states).transpose(-1,-2)).squeeze(-1)
            _, now_indices = torch.topk(score_now,k=min(self.select_len,score_now.shape[-1]),dim = -1,sorted=False)
            now_indices = now_indices.view(shape[0],key_states.shape[1],-1,1).expand(-1,-1,-1,128)
...
class snapkvCache(Cache):
    ...
    def __init__(self,layers=None,device = None,layer_num = 28,maxlen=138100,batch_size=1,prefetch_rate=-1) -> None:
        super().__init__()
        self.batch_size=batch_size
        self.layer_num = layer_num
        self.maxlen = maxlen+128
        self.prefetch_len = 160
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            now_len = int(0.075*score_now.shape[-1])
