...
from quant.myquant import ontbitquant,onebitgemv,extract_sign_and_compress,my_scatter_add,mydequant,my_resduialquant,my_value_quant,my_value_dequant,mykeyquantV2,mykeydequantV2,vq_lutgemv
...
def vq_hash(key_states):
    ...
    powers_of_2 = 2 ** torch.arange(0, 4, 1, device=device)
...
def reconstruct(key_states,codebook):
    ...
    return reconstructed_data_trans.to(types),codebook
class true_selfindex_Cache(Cache):
    """
    ...
    def __init__(self,layers=None,device = None,layer_num = 32,maxlen=138100,batch_size=1,prefetch_rate = 0.15) -> None:
        ...
        self.generated_token = 0
        self.compress_len = 0
        self.codebook:List[torch.Tensor] = []
        self.key_value_quant:List[torch.Tensor] = []
        self.key_1bit_quant:List[torch.Tensor] = []
        self.key_value_quant_param:List[torch.Tensor] = []
        self.key_value_quant_mock:List[torch.Tensor] = []
        self.key_1bit_quant_mock:List[torch.Tensor] = []
        self.key_value_quant_param_mock:List[torch.Tensor] = []
        self.neaset_len = 4
        self.sink_len = 60
        self.select_token = 820
        # print("true sketchkv")
    ...
    def get_same_tensor(self,tensor,extend_len=512):
        shape = tensor.shape 
        lens = shape[-2]

        new_tensor = torch.zeros((shape[0],shape[1],shape[2]+extend_len,shape[3]),dtype = tensor.dtype,device = tensor.device)
        new_tensor[:,:,:lens,:] = tensor
        return new_tensor
    def update(
        self,
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            self.compress_len = need_compress_keycache.shape[-2]
            # self.select_token = int(self.compress_len*0.075)
            self.select_token = min(100, self.compress_len)
            self.abs_key.append(need_compress_keycache.abs().max(dim=-2,keepdim=True)[0])
            key_value_quant,key_1bit_quant,key_value_quant_param = my_key_value_quant_V4(need_compress_keycache/self.abs_key[layer_idx],need_compress_valuecache)
            # buffer1 = get_same_tensor(key_value_quant)
            self.key_value_quant.append(self.get_same_tensor(key_value_quant))
            self.key_1bit_quant.append(self.get_same_tensor(key_1bit_quant))
            self.key_value_quant_param.append(self.get_same_tensor(key_value_quant_param))
            # self.key_value_quant_mock.append(key_value_quant)
            # self.key_1bit_quant_mock.append(key_1bit_quant)
            # self.key_value_quant_param_mock.append(key_value_quant_param)
            self.key_cache.append(full_k/self.abs_key[layer_idx])
            self.value_cache.append(full_v)
            codebook = vq_hash(need_compress_keycache)
            ...
            key_states = key_states-self.means[layer_idx]
            rounds = 2**3-1
            key_value_quant,key_1bit_quant,key_value_quant_param = my_key_value_quant_V4(key_states/self.abs_key[layer_idx],value_states)
...
class oursfullCache(Cache):
    ...
    def __init__(self,layers=None,device = None,layer_num = 32,maxlen=138100,batch_size=1,prefetch_rate = 0.15) -> None:
        ...
        self.compress_value_cache: List[torch.Tensor] = []
        self.means:List[torch.Tensor] = []
        self.q_means:List[torch.Tensor] = []
        self.sign:List[torch.Tensor] = []
        self.abs_key:List[torch.Tensor] = []
        self.abs_value:List[torch.Tensor] = []
        self.abs_query:List[torch.Tensor] = []
    ...
    def update(
        ...
        if len(self.key_cache) <= layer_idx:
            ...
            shape = q_1.shape
            q_1 = q_1.view(shape[0],key_states.shape[1],-1,shape[2],shape[3]).mean(dim=2)
            q_median = torch.median(q_1, dim=-2,keepdim=True)[0]
            self.q_means.append(q_median)
            self.key_cache.append(torch.cat((key_states[:,:,:32,:],key_states[:,:,-32:,:]),dim=-2))
            self.value_cache.append(torch.cat((value_states[:,:,:32,:],value_states[:,:,-32:,:]),dim=-2))
            
            self.compress_key_cache.append(key_states[:,:,32:-32,:].clone())
            self.compress_value_cache.append(value_states[:,:,32:-32,:].clone())
            return key_states,value_states
        else:
            key_states = key_states-self.means[layer_idx]
            rounds = 2**3-1
            ...
            shape = q_1.shape
            q_1 = q_1.view(shape[0],key_states.shape[1],-1,shape[2],shape[3]).mean(dim=2)
            score_now = torch.matmul(torch.sign(self.compress_key_cache[layer_idx]),torch.sign(q_1-self.q_means[layer_idx]).transpose(-1,-2)).squeeze(-1)
            # score_now = score_now.view(shape[0],key_states.shape[1],-1,16).max(dim=-1)[0]
            _, now_indices = torch.topk(score_now,k=min(96,score_now.shape[-1]),dim = -1,sorted=False)
